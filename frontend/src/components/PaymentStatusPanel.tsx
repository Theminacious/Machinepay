import type { PaymentStatus } from "../hooks/usePayment";
import { explorerName, explorerTxUrl } from "../lib/chain";
import { formatMon, shortHash } from "../lib/format";
import { BlockIcon, CheckIcon, LinkIcon, ShieldIcon, SpinnerIcon } from "./Icons";

type Props = {
  status: PaymentStatus;
  chainId: number | undefined;
  onDismiss: () => void;
  /// Re-submits a blocked payment without the pre-flight check, so the refusal
  /// lands on chain as a reverted transaction.
  onProveOnChain?: () => void;
};

function Row({ label, value, tone }: { label: string; value: string; tone?: "bad" | "ok" }) {
  return (
    <div className="flex items-baseline justify-between gap-4 py-1.5">
      <dt className="text-xs text-ink-400">{label}</dt>
      <dd
        className={`tabular text-sm ${
          tone === "bad" ? "text-rose-alert" : tone === "ok" ? "text-mint-300" : "text-ink-200"
        }`}
      >
        {value}
      </dd>
    </div>
  );
}

function TxLink({ hash, chainId }: { hash: `0x${string}`; chainId: number | undefined }) {
  const url = explorerTxUrl(chainId, hash);
  return (
    <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs">
      <span className="tabular text-ink-400">{shortHash(hash)}</span>
      {url ? (
        <a
          href={url}
          target="_blank"
          rel="noreferrer"
          className="focus-ring inline-flex items-center gap-1.5 rounded text-mint-300 underline decoration-mint-500/40 underline-offset-2 transition hover:text-mint-200"
        >
          View on {explorerName(chainId)}
          <LinkIcon className="h-3 w-3" />
        </a>
      ) : (
        <span className="text-ink-500">local chain — no explorer</span>
      )}
    </div>
  );
}

export function PaymentStatusPanel({ status, chainId, onDismiss, onProveOnChain }: Props) {
  const { phase, intent, hash, rejection, preflightSkipped } = status;
  if (phase === "idle") return null;

  const route = intent ? `${intent.from} → ${intent.to}` : "";

  /// The pre-flight check is a courtesy, not the enforcement point. Saying so
  /// keeps the distinction honest when the endpoint is the thing that failed.
  const skippedNote = preflightSkipped ? (
    <p className="mt-2 text-xs text-amber-glow">
      {preflightSkipped.headline} — the policy pre-check could not run, so the contract checked this payment itself.
    </p>
  ) : null;

  if (phase === "checking" || phase === "signing" || phase === "pending") {
    const copy = {
      checking: { title: "Checking spending rules", body: "Asking the contract whether this payment is permitted." },
      signing: { title: "Waiting for your signature", body: "Approve the payment in your wallet." },
      pending: { title: "Payment in flight", body: "Submitted to the network, waiting for confirmation." },
    }[phase];

    return (
      <section className="panel animate-rise border-mint-500/25 p-5">
        <div className="flex items-start gap-3">
          <SpinnerIcon className="mt-0.5 h-5 w-5 shrink-0 text-mint-400" />
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-semibold text-white">{copy.title}</h3>
            <p className="mt-0.5 text-xs text-ink-400">{copy.body}</p>
            {intent && (
              <p className="tabular mt-2.5 text-sm text-ink-200">
                {route} · {formatMon(intent.amount)} MON
              </p>
            )}
            {hash && <TxLink hash={hash} chainId={chainId} />}
            {skippedNote}
          </div>
        </div>
      </section>
    );
  }

  if (phase === "confirmed") {
    return (
      <section className="panel animate-rise border-mint-500/35 bg-mint-900/20 p-5">
        <div className="flex items-start gap-3">
          <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-mint-500/15 text-mint-300">
            <CheckIcon className="h-3.5 w-3.5" />
          </span>
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-sm font-semibold text-white">Payment confirmed</h3>
              <button
                type="button"
                onClick={onDismiss}
                className="focus-ring rounded text-xs text-ink-400 transition hover:text-ink-200"
              >
                Dismiss
              </button>
            </div>
            <p className="tabular mt-1.5 text-sm text-ink-200">
              {intent?.label} · {route} · {intent ? formatMon(intent.amount) : "—"} MON
            </p>
            {status.blockNumber !== undefined && (
              <p className="tabular mt-1 text-xs text-ink-400">block {status.blockNumber.toString()}</p>
            )}
            {hash && <TxLink hash={hash} chainId={chainId} />}
            {skippedNote}
          </div>
        </div>
      </section>
    );
  }

  // Blocked or failed. The policy case is the one that matters: the contract
  // refused, and we show its own numbers back.
  const isPolicy = rejection?.kind === "policy";
  return (
    <section
      className={`panel animate-shake p-5 ${
        isPolicy ? "border-rose-alert/45 bg-rose-alert/[0.07]" : "border-ink-600"
      }`}
    >
      <div className="flex items-start gap-3">
        <span
          className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full ${
            isPolicy ? "bg-rose-alert/15 text-rose-alert" : "bg-ink-800 text-ink-300"
          }`}
        >
          <BlockIcon className="h-3.5 w-3.5" />
        </span>

        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <h3
              className={`text-sm font-semibold tracking-wide ${isPolicy ? "text-rose-alert uppercase" : "text-white"}`}
            >
              {rejection?.headline ?? "Transaction blocked"}
            </h3>
            <button
              type="button"
              onClick={onDismiss}
              className="focus-ring rounded text-xs text-ink-400 transition hover:text-ink-200"
            >
              Dismiss
            </button>
          </div>
          <p className="mt-0.5 text-sm text-ink-200">{rejection?.reason}</p>
          {rejection?.kind === "rpc" && (
            <p className="mt-1 text-xs text-amber-glow">
              This is the RPC endpoint, not a spending policy. No payment was sent and no rule refused it.
            </p>
          )}

          <dl className="mt-3.5 divide-y divide-ink-700/60 border-y border-ink-700/60">
            {rejection?.requested !== undefined && (
              <Row label="Requested" value={`${formatMon(rejection.requested)} MON`} tone="bad" />
            )}
            {rejection?.allowed !== undefined && (
              <Row
                label={
                  // Whatever number refused this payment gets named for what it
                  // actually is, so no row ever reads as the wrong rule.
                  rejection.errorName === "DailyLimitExceeded"
                    ? "Left in today's budget"
                    : rejection.kind === "funds"
                      ? "Available"
                      : "Maximum allowed"
                }
                value={`${formatMon(rejection.allowed)} MON`}
              />
            )}
            {rejection?.machineId && <Row label="Machine" value={rejection.machineId} />}
            {rejection?.errorName && <Row label="Rule" value={rejection.errorName} />}
          </dl>

          {rejection?.detail && <p className="mt-3 text-xs text-ink-400">{rejection.detail}</p>}

          {isPolicy && (
            <p className="mt-3 inline-flex items-center gap-2 rounded-lg border border-mint-500/25 bg-mint-900/30 px-3 py-2 text-xs text-mint-300">
              <ShieldIcon className="h-3.5 w-3.5 shrink-0" />
              Funds protected — no balance moved.
            </p>
          )}

          {hash && <TxLink hash={hash} chainId={chainId} />}

          {status.provenOnChain ? (
            <p className="mt-2 text-xs text-ink-400">
              Submitted anyway: the transaction is on chain and reverted. Nothing was transferred.
            </p>
          ) : (
            isPolicy &&
            onProveOnChain && (
              <button
                type="button"
                onClick={onProveOnChain}
                className="focus-ring mt-3 rounded-lg border border-ink-600 px-3 py-1.5 text-xs text-ink-300 transition hover:border-ink-400 hover:text-ink-100"
              >
                Submit it anyway → prove the refusal on chain
              </button>
            )
          )}
        </div>
      </div>
    </section>
  );
}
